...
# -*- coding: utf-8 -*-
#
# 自包含单文件：数据集组合 sweep + AMOTF NPZ 构建，不依赖同目录其它训练/核心模块。
#
# === 必须先读：顺序与产物路径 ===
# - dataset_sweep 只使用「已存在且校验通过」的 amotf_npz，不会自动替你 build。
# - 请先用本脚本的 build_amotf（或与 sweep 相同的 --max_points）生成文件，再跑 dataset_sweep。
# - 每个样本需: {original_path}/curve.csv；生成后在:
#     {original_path}/amotf/amotf_charge.npz
#     {original_path}/amotf/amotf_discharge.npz
#   若 build 时 --max_points 不是 2000，则文件名为 amotf_charge_mp{max_points}.npz（放电同理），
#   sweep/train 也必须传相同 --max_points，否则会判为 missing npz。
# - 若 usable_records=0 且 dropped 含 missing_or_invalid_amotf_charge_or_discharge_npz，说明尚未构建或路径/max_points 不一致。
#
# === 脚本用途 ===
# - build_amotf: 按 labels_csv 的 original_path 批量构建 AMOTF（NPZ + PNG）到各样本 amotf/ 目录。
# - dataset_sweep: 按内建顺序逐步加入数据集，联合分层（dataset×class）训练，结果写入 run 目录下 CSV。
#
# === 命令行（复制后按需改路径；续行末尾反斜杠后勿再跟字符）===
#
# --- 帮助 ---
# python soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py --help
# python soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py build_amotf --help
# python soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py dataset_sweep --help
#
# --- build_amotf：默认参数（与 sweep 默认 max_points=2000 对齐）---
# python /mnt/sdb/THX/Battery_THX_HP_P9000/Battery/dataset/Tao/Battery_Archive/scripts/battery-soh-dino/train/soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py build_amotf \
#   --labels_csv /mnt/sdb/THX/Battery_THX_HP_P9000/no_title_outputs/features/soh_classification_results.csv
#
# --- build_amotf：显式常用参数（多进程 / 覆盖已有）---
# python /mnt/sdb/THX/Battery_THX_HP_P9000/Battery/dataset/Tao/Battery_Archive/scripts/battery-soh-dino/train/soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py build_amotf \
#   --labels_csv /mnt/sdb/THX/Battery_THX_HP_P9000/no_title_outputs/features/soh_classification_results.csv \
#   --max_points 2000 \
#   --m 5 \
...
#   --num_workers 4
#
# --- dataset_sweep：完整训练相关参数（epoch 与 epochs 等价，见 argparse dest）---
# python /mnt/sdb/THX/Battery_THX_HP_P9000/Battery/dataset/Tao/Battery_Archive/scripts/battery-soh-dino/train/soh_dino_amotf_npz_dataset_cycles_all_classes_soc_horizontal.py dataset_sweep \
#   --labels_csv /mnt/sdb/THX/Battery_THX_HP_P9000/no_title_outputs/features/soh_classification_results.csv \
#   --runs_root /mnt/sdb/THX/Battery_THX_HP_P9000/no_title_outputs/soh_amotf_dino_runs \
#   --run_name exp_dataset_sweep_soc_horizontal \
#   --max_points 2000 \
#   --kfold 5 \
#   --seed 42 \
#   --img_size 224 \
...
#
# --- dataset_sweep：仅离线（需已缓存 HF 权重）在上一段末尾追加一行 ---
#   --hf_local_only
#
# --- 可选：LoRA / 换骨干 / 提高每步最少样本阈值 ---
...
SWEEP_LOGGER = logging.getLogger("soh_amotf_dino_dataset_sweep")
...
def _setup_sweep_logging(log_path: Path, level: str = "INFO") -> None:
    _ensure_dir(log_path.parent)
    lvl = getattr(logging, str(level).upper(), logging.INFO)
    SWEEP_LOGGER.setLevel(lvl)
    ...
    SWEEP_LOGGER.handlers.clear()
    SWEEP_LOGGER.addHandler(fh)
    SWEEP_LOGGER.addHandler(sh)
...
def run_dataset_sweep(
    ...
    **_: Any,
    ...
    SWEEP_LOGGER.info("target_datasets=%s", TARGET_DATASETS)

    records_all, dropped0 = _load_records(labels_csv, max_points=int(max_points))
    records, dropped1 = _filter_by_input_mode(records_all, input_mode="amotf_npz")

    dropped = dict(dropped0)
    for k, v in dropped1.items():
        dropped[k] = dropped.get(k, 0) + int(v)

    SWEEP_LOGGER.info("loaded_records=%d usable_records=%d dropped=%s", len(records_all), len(records), dropped)
    ...
    for st in SWEEP_STEPS:
        ...
        joint_keys = _make_joint_keys(subset_records)
        uniq, cnt = np.unique(joint_keys, return_counts=True)
        min_cnt = int(cnt.min()) if cnt.size else 0
    ...
    SWEEP_LOGGER.info("done. results_csv=%s", str(results_csv))
    return 0
